<!-- Route view for `/shaper/queues` in the ezNGFW admin GUI. -->
...
<div class="space-y-6 bg-gradient-to-b from-slate-950 via-slate-950 to-slate-900 p-4 text-slate-100 md:p-6">
  <ResourceTable
    title={$_('shaper_queues.titletraffic_shaper_queues')}
    description={$_('shaper_queues.descriptionqueue_hierarchy_management_with_rich_fi')}
    endpoint={endpoint}
    columns={columns}
    fields={fields}
    idKey="id"
    addLabel={$_('shaper_queues.addlabeladd_queue')}
  />
</div>
