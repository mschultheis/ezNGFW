<!-- Route view for `/ipv6` in the ezNGFW admin GUI. -->
...
<script lang="ts">
  ...
  const secondaryTables = [
    {
        "endpoint": "/ipv6/interfaces",
        "title": "IPv6 Interface Status",
        "description": "Per-interface IPv6 addresses and runtime state.",
        "columns": [
            "interface",
            "ipv6Address",
            "prefix",
    ...
    {
        "endpoint": "/ipv6/routes",
        "title": "IPv6 Route Overview",
        "description": "Operational IPv6 route table summary.",
        "columns": [
            "destination",
            "nextHop",
            "interface",
  ...
  async function loadSettings() {
    try {
      const payload = await api.get<Record<string, any>>('/ipv6/settings');
      ...
      toasts.error($_('ipv6.toastfailed_to_load_page_settings'));
    }
  }

  async function loadRows() {
    try {
      const payload = await api.get<unknown[]>('/ipv6/profiles');
  ...
  async function saveSettings() {
    ...
    try {
      ...
      await api.put('/ipv6/settings', payload);
      toasts.success($_('ipv6.toastsettings_saved'));
    } catch {
      toasts.error($_('ipv6.toastfailed_to_save_settings'));
    } finally {
      saving = false;
    }
  }
...
<div class="space-y-6">
  <div class="grid gap-4 md:grid-cols-3">
    <Card class="border-slate-800 bg-slate-900/60">
      <CardHeader class="pb-2"><CardTitle class="text-sm text-slate-200">{$_('ipv6.active_profiles')}</CardTitle></CardHeader>
      <CardContent><p class="text-2xl font-semibold text-cyan-400">{activeCount}</p></CardContent>
    </Card>
    <Card class="border-slate-800 bg-slate-900/60">
      <CardHeader class="pb-2"><CardTitle class="text-sm text-slate-200">{$_('ipv6.disabled_profiles')}</CardTitle></CardHeader>
      <CardContent><p class="text-2xl font-semibold text-amber-300">{disabledCount}</p></CardContent>
    </Card>
    <Card class="border-slate-800 bg-slate-900/60">
      <CardHeader class="pb-2"><CardTitle class="text-sm text-slate-200">{$_('ipv6.total_records')}</CardTitle></CardHeader>
  ...
  <Card class="border-slate-800 bg-slate-900/60">
    <CardHeader>
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <CardTitle class="text-slate-100">{$_('ipv6.ipv6_control_plane_management')}</CardTitle>
          <CardDescription class="text-slate-400">{$_('ipv6.coordinate_ipv6_stack_settings_advertisement_polic')}</CardDescription>
        </div>
    ...
    <CardContent class="space-y-6">
      {#if loading}
        <div class="rounded-md border border-slate-800 bg-slate-950/50 px-4 py-8 text-center text-sm text-slate-400">Loading IPv6 configuration...</div>
      {:else}
        ...
        <Collapsible.Root bind:open={showAdvanced} class="pt-1">
          <Collapsible.Trigger class="flex w-full items-center justify-between rounded-md border border-slate-800 bg-slate-950/40 px-3 py-2">
            <span class="font-medium text-slate-200">{$_('ipv6.advanced_operational_behavior')}</span>
  ...
  <ResourceTable
    title={$_('ipv6.titleipv6_profiles')}
    description={$_('ipv6.descriptionconfigure_ipv6_addressing_router_advert')}
    endpoint="/ipv6/profiles"
    columns={columns}
    fields={fields}
    idKey="id"
    addLabel={$_('ipv6.addlabeladd_profile')}
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              {#if (secondaryData[section.endpoint] ?? []).length === 0}
                <tr><td class="px-3 py-6 text-center text-slate-500" colspan={section.columns.length}>No IPv6 records found.</td></tr>
              {:else}
