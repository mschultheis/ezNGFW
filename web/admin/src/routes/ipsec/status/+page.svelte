<!-- Route view for `/ipsec/status` in the ezNGFW admin GUI. -->
...
<script lang="ts">
  ...
  const secondaryTables = [
  {
    ...
    "title": "Tunnel Status",
    "description": "Live IKE and Child SA counters for all configured tunnels.",
    "columns": [
      "tunnel_name",
      "ike_state",
      "child_sa_count",
  ...
  {
    "endpoint": "/ipsec/tunnels",
    "title": "Phase Definitions",
    "description": "Configured Phase 1 and Phase 2 policy details.",
    "columns": [
      "name",
      "phase1",
      "phase2",
  ...
  const settingsEndpoint = '/ipsec/status/settings';
  const rowsEndpoint = '/ipsec/status/watch';
  ...
  async function loadSettings(): Promise<void> {
    try {
      ...
      toasts.error($_('ipsec_status.toastfailed_to_load_settings'));
    }
  }
  ...
  async function loadRows(): Promise<void> {
    ...
    try {
      ...
      rows = [];
      toasts.error($_('ipsec_status.toastfailed_to_load_rows'));
    } finally {
      loadingRows = false;
    }
  }
  ...
  async function saveSettings(): Promise<void> {
    if (!canSaveSettings) {
      toasts.error($_('ipsec_status.toastresolve_settings_validation_errors_before_sav'));
      return;
    }
    ...
    try {
      ...
      toasts.success($_('ipsec_status.toastsettings_saved'));
      await loadAll();
    } catch {
      toasts.error($_('ipsec_status.toastfailed_to_save_settings'));
    } finally {
      saving = false;
    }
  }
  ...
  async function saveRow(): Promise<void> {
    if (!canSaveRow) {
      toasts.error($_('ipsec_status.toastresolve_row_validation_errors_before_saving'));
      return;
    }
    ...
    try {
      ...
      toasts.success(editingId ? 'IPsec watch entry updated' : 'IPsec watch entry added');
      resetDraft();
      await Promise.all([loadRows(), loadSecondary()]);
    } catch {
      toasts.error($_('ipsec_status.toastfailed_to_save_row'));
    } finally {
      saving = false;
    }
  }
  ...
  async function deleteRow(id: string): Promise<void> {
    ...
    try {
      ...
      toasts.success($_('ipsec_status.toastipsec_watch_entry_deleted'));
      await Promise.all([loadRows(), loadSecondary()]);
    } catch {
      toasts.error($_('ipsec_status.toastfailed_to_delete_row'));
    } finally {
      saving = false;
    }
  }
...
<div class="space-y-6 bg-gradient-to-b from-slate-950 via-slate-925 to-slate-950 p-4 text-slate-100 md:p-6">
  <Card class="border-slate-800 bg-slate-900/70">
    <CardHeader>
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <CardTitle class="text-slate-100">{$_('ipsec_status.ipsec_tunnel_and_sa_status')}</CardTitle>
          <CardDescription class="text-slate-400">
            Track IKE/Child SA watch entries with richer operator context and sortable inline controls.
          </CardDescription>
        </div>
    ...
    <CardContent class="space-y-5">
      <div class="grid gap-3 md:grid-cols-4">
        <div class="rounded-md border border-slate-800 bg-slate-950/60 p-3">
          <p class="text-xs text-slate-400">{$_('ipsec_status.total_rows')}</p>
          <p class="text-lg font-semibold text-slate-100">{rowCount}</p>
        </div>
        <div class="rounded-md border border-slate-800 bg-slate-950/60 p-3">
          <p class="text-xs text-slate-400">{$_('ipsec_status.enabled')}</p>
          <p class="text-lg font-semibold text-emerald-300">{enabledCount}</p>
        </div>
        <div class="rounded-md border border-slate-800 bg-slate-950/60 p-3">
          <p class="text-xs text-slate-400">{$_('ipsec_status.disabled')}</p>
          <p class="text-lg font-semibold text-amber-300">{disabledCount}</p>
        </div>
        <div class="rounded-md border border-slate-800 bg-slate-950/60 p-3">
          <p class="text-xs text-slate-400">{$_('ipsec_status.last_refresh')}</p>
        ...
        {#if settingErrorList.length > 0}
          <div class="rounded-md border border-red-500/40 bg-red-500/10 p-3 text-sm text-red-200">
            <div class="mb-2 flex items-center gap-2">
              ...
              <span>{$_('ipsec_status.resolve_these_settings_validation_issues_before_sa')}</span>
            </div>
  ...
  <Card class="border-slate-800 bg-slate-900/70">
    <CardHeader>
      <CardTitle class="text-slate-100">{$_('ipsec_status.ipsec_watch_entries')}</CardTitle>
      <CardDescription class="text-slate-400">
    ...
    <CardContent class="space-y-4">
      <div class="grid gap-3 md:grid-cols-4">
        <Input
          ...
          placeholder={$_('ipsec_status.placeholdersearch_rows_by_any_field')}
          value={search}
          oninput={(event) => {
