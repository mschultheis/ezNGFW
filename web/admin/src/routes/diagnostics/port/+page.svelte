<!-- Route view for `/diagnostics/port` in the ezNGFW admin GUI. -->
...
<script lang="ts">
  ...
  const secondaryTables = [
    {
        "endpoint": "/diagnostics/port/history",
        "title": "Probe History",
        "description": "Recent probe outcomes and timing details.",
        "columns": [
            "timestamp",
            "target",
            "protocol",
    ...
    {
        "endpoint": "/diagnostics/port/health",
        "title": "Probe Health Summary",
        "description": "Aggregated pass/fail rates for key probes.",
        "columns": [
            "profile",
            "successRate",
            "avgLatency",
  ...
  async function loadSettings() {
    try {
      const payload = await api.get<Record<string, any>>('/diagnostics/port/settings');
      ...
      toasts.error($_('diagnostics_port.toastfailed_to_load_page_settings'));
    }
  }
  ...
  async function loadRows() {
    loadingRows = true;
    try {
      const payload = await api.get<unknown[]>('/diagnostics/port/profiles');
      rows = Array.isArray(payload) ? payload.map((entry) => normalizeRecord(entry, rowFields)) : [];
    } catch {
      rows = [];
      toasts.error($_('diagnostics_port.toastfailed_to_load_inline_records'));
    } finally {
      loadingRows = false;
    }
  }
  ...
  async function saveSettings() {
    ...
    try {
      ...
      await api.put('/diagnostics/port/settings', payload);
      toasts.success($_('diagnostics_port.toastsettings_saved'));
    } catch {
      toasts.error($_('diagnostics_port.toastfailed_to_save_settings'));
    } finally {
      saving = false;
    }
  }
  ...
  async function saveRow() {
    if (!validateDraft()) {
      toasts.error($_('diagnostics_port.toastplease_resolve_validation_errors_before_savin'));
      return;
    }
    ...
    try {
      ...
      if (editingId) {
        await api.put(`/diagnostics/port/profiles/${editingId}`, payload);
      } else {
        await api.post('/diagnostics/port/profiles', payload);
      }
      toasts.success(editingId ? 'Profile updated' : 'Profile added');
      resetDraft();
      await Promise.all([loadRows(), loadSecondary()]);
    } catch {
      toasts.error($_('diagnostics_port.toastfailed_to_save_profile'));
    } finally {
      saving = false;
    }
  }
  ...
  async function deleteRow(id: string) {
    deleting = true;
    try {
      await api.del(`/diagnostics/port/profiles/${id}`);
      toasts.success($_('diagnostics_port.toastprofile_deleted'));
      await Promise.all([loadRows(), loadSecondary()]);
    } catch {
      toasts.error($_('diagnostics_port.toastfailed_to_delete_profile'));
    } finally {
      deleting = false;
    }
  }
...
<div class="space-y-6">
  <div class="grid gap-4 md:grid-cols-3">
    <Card class="border-slate-800 bg-slate-900/60">
      <CardHeader class="pb-2"><CardTitle class="text-sm text-slate-200">{$_('diagnostics_port.active_profiles')}</CardTitle></CardHeader>
      <CardContent><p class="text-2xl font-semibold text-cyan-400">{activeCount}</p></CardContent>
    </Card>
    <Card class="border-slate-800 bg-slate-900/60">
      <CardHeader class="pb-2"><CardTitle class="text-sm text-slate-200">{$_('diagnostics_port.disabled_profiles')}</CardTitle></CardHeader>
      <CardContent><p class="text-2xl font-semibold text-amber-300">{disabledCount}</p></CardContent>
    </Card>
    <Card class="border-slate-800 bg-slate-900/60">
      <CardHeader class="pb-2"><CardTitle class="text-sm text-slate-200">{$_('diagnostics_port.validation_issues')}</CardTitle></CardHeader>
  ...
  <Card class="border-slate-800 bg-slate-900/60">
    <CardHeader>
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <CardTitle class="text-slate-100">{$_('diagnostics_port.port_diagnostics_and_probe_profiles')}</CardTitle>
          <CardDescription class="text-slate-400">{$_('diagnostics_port.create_reusable_network_probe_definitions_with_adv')}</CardDescription>
        </div>
    ...
    <CardContent class="space-y-6">
      {#if loading}
        <div class="rounded-md border border-slate-800 bg-slate-950/50 px-4 py-8 text-center text-sm text-slate-400">Loading port scan results...</div>
      {:else}
        ...
        <Collapsible.Root bind:open={showAdvanced} class="pt-1">
          <Collapsible.Trigger class="flex w-full items-center justify-between rounded-md border border-slate-800 bg-slate-950/40 px-3 py-2">
            <span class="font-medium text-slate-200">{$_('diagnostics_port.advanced_operational_behavior')}</span>
  ...
  <Card class="border-slate-800 bg-slate-900/60">
    <CardHeader>
      <CardTitle class="text-slate-100">{$_('diagnostics_port.port_probes')}</CardTitle>
      <CardDescription class="text-slate-400">{$_('diagnostics_port.create_validate_edit_and_delete_records_directly_w')}</CardDescription>
    </CardHeader>
    <CardContent class="space-y-4">
      <div class="grid gap-3 md:grid-cols-5">
        <Input class="border-slate-700 bg-slate-950 text-slate-100 md:col-span-2" placeholder={$_('diagnostics_port.placeholdersearch_profiles_by_any_field')} value={search} oninput={(event) => (search = (event.currentTarget as HTMLInputElement).value)} />
      ...
      {#if Object.keys(validation).length > 0}
        <div class="rounded-md border border-rose-500/40 bg-rose-950/20 px-3 py-2 text-sm text-rose-200">
          Fix highlighted fields before saving this probe profile. Validation runs inline to prevent malformed profile payloads.
        </div>
      {/if}
      ...
      <div class="rounded-md border border-slate-800 bg-slate-950/40 p-3">
        <button type="button" class="flex w-full items-center justify-between text-sm text-slate-200" onclick={() => (showFormAdvanced = !showFormAdvanced)}>
          <span>{$_('diagnostics_port.advanced_settings_for_probe_profile_profile')}</span>
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              {#if (secondaryData[section.endpoint] ?? []).length === 0}
                <tr><td class="px-3 py-6 text-center text-slate-500" colspan={section.columns.length}>No port scan results found.</td></tr>
              {:else}
