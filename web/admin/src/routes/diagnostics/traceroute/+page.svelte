<!-- Route view for `/diagnostics/traceroute` in the ezNGFW admin GUI. -->
...
<script lang="ts">
  ...
  const settingsEndpoint = "/diagnostics/traceroute/settings";
  const rowsEndpoint = "/diagnostics/traceroute/profiles";
  const actionEndpoint = "/diagnostics/traceroute";
  ...
  const settingsFields: Field[] = [
  {
    ...
    "kind": "boolean",
    "hint": "Auto Refresh History controls how this workflow behaves in production and during troubleshooting. For example, keep recent traces updating during incident calls and disable during low-touch audits."
  },
  {
    "key": "refreshSeconds",
    "label": "Refresh Interval Seconds",
    "kind": "number",
    "hint": "Refresh Interval Seconds controls how this workflow behaves in production and during troubleshooting. For example, use 5 seconds for active war rooms and 30 seconds for normal operations."
  },
  {
    "key": "defaultProtocol",
    ...
    "hint": "Default Protocol controls how this workflow behaves in production and during troubleshooting. For example, pick udp for internet path testing and tcp when firewalls suppress icmp."
  },
  {
    "key": "resolveHostnames",
    "label": "Resolve Hop Hostnames",
    "kind": "boolean",
    "hint": "Resolve Hop Hostnames controls how this workflow behaves in production and during troubleshooting. For example, enable it for readability, disable it to avoid DNS lookup delays."
  },
  {
    "key": "showPerHopLoss",
    "label": "Show Per-Hop Loss",
    "kind": "boolean",
    "hint": "Show Per-Hop Loss controls how this workflow behaves in production and during troubleshooting. For example, turn this on when identifying packet drop patterns across transit providers."
  },
  {
    "key": "preserveHistory",
    "label": "Preserve Run History",
    "kind": "boolean",
    "hint": "Preserve Run History controls how this workflow behaves in production and during troubleshooting. For example, retain traces for before/after comparisons during carrier escalations."
  }
];
  const actionFields: Field[] = [
  {
    ...
    "label": "Target Host",
    "kind": "text",
    "hint": "Target Host controls how this workflow behaves in production and during troubleshooting. For example, test app.example.com to verify application path viability from the firewall."
  },
  {
    "key": "maxHops",
    "label": "Maximum Hops",
    "kind": "number",
    "hint": "Maximum Hops controls how this workflow behaves in production and during troubleshooting. For example, set 30 for internet tests and reduce to 8 for local WAN edge troubleshooting."
  },
  {
    "key": "protocol",
    ...
    "hint": "Probe Protocol controls how this workflow behaves in production and during troubleshooting. For example, switch to tcp when middleboxes prioritize or filter icmp traffic."
  },
  {
    "key": "port",
    "label": "Destination Port",
    "kind": "number",
    "hint": "Destination Port controls how this workflow behaves in production and during troubleshooting. For example, use 443 for HTTPS path behavior and 53 for DNS transit checks."
  },
  {
    "key": "interface",
    ...
    "hint": "Source Interface controls how this workflow behaves in production and during troubleshooting. For example, force WAN2 to validate policy-routing and failover egress expectations."
  },
  {
    "key": "resolve",
    "label": "Resolve Hop Names",
    "kind": "boolean",
    "hint": "Resolve Hop Names controls how this workflow behaves in production and during troubleshooting. For example, disable this for raw speed during outages and enable for easier reporting."
  }
];
  const rowFields: Field[] = [
  {
    ...
    "kind": "text",
    "required": true,
    "hint": "Profile Name controls how this workflow behaves in production and during troubleshooting. For example, create a profile like Branch-ISP-Path for repeated post-change validation."
  },
  {
    "key": "host",
    "label": "Target Host",
    "kind": "text",
    "required": true,
    "hint": "Target Host controls how this workflow behaves in production and during troubleshooting. For example, save dc-gateway.example.net to standardize recurring checks."
  },
  {
    "key": "protocol",
    "label": "Protocol",
    ...
    "hint": "Protocol controls how this workflow behaves in production and during troubleshooting. For example, store tcp profiles for application-aware path diagnostics."
  },
  {
    "key": "maxHops",
    "label": "Max Hops",
    "kind": "number",
    "hint": "Max Hops controls how this workflow behaves in production and during troubleshooting. For example, use lower values for internal segments and higher values for internet paths."
  },
  {
    "key": "interface",
    "label": "Interface",
    ...
    "hint": "Interface controls how this workflow behaves in production and during troubleshooting. For example, bind profile to WAN1 or WAN2 for deterministic testing."
  },
  {
    "key": "enabled",
    "label": "Enabled",
    "kind": "boolean",
    "hint": "Enabled controls how this workflow behaves in production and during troubleshooting. For example, disable stale profiles instead of deleting to preserve operator context."
  },
  {
    "key": "description",
    "label": "Description",
    "kind": "textarea",
    "hint": "Description controls how this workflow behaves in production and during troubleshooting. For example, record expected hop patterns so deviations are obvious to on-call staff."
  }
];
  const secondaryTables: { endpoint: string; title: string; description: string; columns: string[] }[] = [
  {
    "endpoint": "/diagnostics/traceroute/history",
    "title": "Recent Traceroute Runs",
    "description": "Stored run summaries with status and elapsed time for trend and change impact analysis.",
    "columns": [
      "timestamp",
      "host",
      "protocol",
  ...
  {
    "endpoint": "/diagnostics/traceroute/hops",
    "title": "Hop RTT Breakdown",
    "description": "Per-hop latency snapshots including average round-trip values for bottleneck detection.",
    "columns": [
      "hop",
      "address",
      "rtt1",
  ...
  async function loadSettings() {
    ...
    try {
      ...
      toasts.error($_('diagnostics_traceroute.toastfailed_to_load_configuration_settings'));
    }
  }
  ...
  async function loadRows() {
    ...
    try {
      ...
      rows = [];
      toasts.error($_('diagnostics_traceroute.toastfailed_to_load_records'));
    } finally {
      loadingRows = false;
    }
  }
  ...
  async function saveSettings() {
    ...
    try {
      ...
      toasts.success($_('diagnostics_traceroute.toastsettings_saved'));
    } catch {
      toasts.error($_('diagnostics_traceroute.toastfailed_to_save_settings'));
    } finally {
      saving = false;
    }
  }
  ...
  async function runPrimaryAction() {
    ...
    try {
      ...
      toasts.success($_('diagnostics_traceroute.toastaction_completed_successfully'));
      await loadSecondary();
    } catch {
      actionResults = [];
      actionOutput = 'Request failed. Check backend service status and try again.';
      toasts.error($_('diagnostics_traceroute.toastaction_failed'));
    } finally {
      runningAction = false;
    }
  }
  ...
  async function saveRow() {
    ...
    try {
      ...
      toasts.error($_('diagnostics_traceroute.toastfailed_to_save_record'));
    } finally {
      saving = false;
    }
  }
  ...
  async function deleteRow(id: string) {
    ...
    try {
      ...
      toasts.success($_('diagnostics_traceroute.toastrecord_deleted'));
      await Promise.all([loadRows(), loadSecondary()]);
    } catch {
      toasts.error($_('diagnostics_traceroute.toastfailed_to_delete_record'));
    } finally {
      saving = false;
    }
  }
  ...
  const summaryCards = $derived.by(() => [
    { label: 'Total Traces', value: String(rows.length) },
    { label: 'Showing', value: String(filteredRows.length) },
    { label: 'Trace Results', value: String(actionResults.length) },
...
<div class="space-y-6">
  <Card class="border-slate-800 bg-slate-900/60">
    <CardHeader>
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <CardTitle class="text-slate-100">{$_('diagnostics_traceroute.traceroute_diagnostics')}</CardTitle>
          <CardDescription class="text-slate-400">{$_('diagnostics_traceroute.run_hopbyhop_path_analysis_with_protocol_and_hop_c')}</CardDescription>
        </div>
    ...
    <CardContent class="space-y-6">
      ...
      {#if loading}
        <div class="rounded-md border border-slate-800 bg-slate-950/50 px-4 py-8 text-center text-sm text-slate-400">Loading traceroute results...</div>
      {:else}
        ...
        <Collapsible.Root bind:open={showAdvanced} class="pt-1">
          <Collapsible.Trigger>
            <span class="font-medium text-slate-200">{$_('diagnostics_traceroute.advanced_operational_behavior')}</span>
  ...
  <Card class="border-slate-800 bg-slate-900/60">
    <CardHeader>
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <CardTitle class="text-slate-100">{$_('diagnostics_traceroute.live_traceroute_runner')}</CardTitle>
          <CardDescription class="text-slate-400">{$_('diagnostics_traceroute.probe_remote_paths_hopbyhop_and_capture_rtt_detail')}</CardDescription>
        </div>
        <Button class="bg-cyan-600 text-white hover:bg-cyan-700" onclick={runPrimaryAction} disabled={runningAction || loading || !actionEndpoint}>
          <Play class="mr-2 h-4 w-4" /> {runningAction ? 'Running...' : 'Start Traceroute'}
        </Button>
      </div>
    </CardHeader>
    <CardContent class="space-y-4">
      ...
      <Collapsible.Root bind:open={showToolHints}>
        <Collapsible.Trigger>
          <span class="font-medium text-slate-200">{$_('diagnostics_traceroute.how_to_interpret_tool_output')}</span>
      ...
      <div class="rounded-md border border-slate-800 bg-slate-950/50 p-3">
        <p class="mb-2 text-xs uppercase tracking-wide text-slate-400">{$_('diagnostics_traceroute.live_output')}</p>
  ...
  <Card class="border-slate-800 bg-slate-900/60">
    <CardHeader>
      <CardTitle class="text-slate-100">{$_('diagnostics_traceroute.traceroute_targets')}</CardTitle>
      <CardDescription class="text-slate-400">{$_('diagnostics_traceroute.create_edit_and_delete_records_directly_in_place_t')}</CardDescription>
    </CardHeader>
    <CardContent class="space-y-4">
      <div class="grid gap-3 md:grid-cols-3">
        <Input class="border-slate-700 bg-slate-950 text-slate-100" placeholder={$_('diagnostics_traceroute.placeholdersearch_records')} value={search} oninput={(event) => (search = (event.currentTarget as HTMLInputElement).value)} />
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              {#if (secondaryData[section.endpoint] ?? []).length === 0}
                <tr><td class="px-3 py-6 text-center text-slate-500" colspan={section.columns.length}>No traceroute results found.</td></tr>
              {:else}
