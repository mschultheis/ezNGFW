<!-- Route view for `/dhcp/status` in the ezNGFW admin GUI. -->
...
<script lang="ts">
  ...
  const settingsEndpoint = "/dhcp/settings";
  const rowsEndpoint = "/dhcp/pools";
  const actionEndpoint = "/dhcp/restart";
  ...
  const settingsFields: Field[] = [
  {
    ...
    "kind": "boolean",
    "hint": "Auto Refresh Status controls how this workflow behaves in production and during troubleshooting. For example, enable during maintenance windows to observe restart and recovery behavior live."
  },
  {
    "key": "refreshSeconds",
    "label": "Refresh Interval Seconds",
    "kind": "number",
    "hint": "Refresh Interval Seconds controls how this workflow behaves in production and during troubleshooting. For example, choose 10 seconds while troubleshooting and 60 seconds for normal visibility."
  },
  {
    "key": "backend",
    "label": "DHCP Backend",
    ...
    "hint": "DHCP Backend controls how this workflow behaves in production and during troubleshooting. For example, select kea for modern lease APIs or dnsmasq for lightweight deployments."
  },
  {
    "key": "showPoolUtilization",
    "label": "Show Pool Utilization",
    "kind": "boolean",
    "hint": "Show Pool Utilization controls how this workflow behaves in production and during troubleshooting. For example, keep this enabled to detect address exhaustion before client impact."
  },
  {
    "key": "showLeaseAging",
    "label": "Show Lease Aging",
    "kind": "boolean",
    "hint": "Show Lease Aging controls how this workflow behaves in production and during troubleshooting. For example, enable aging to identify stale clients and reclaimable addresses."
  },
  {
    "key": "notifyOnFailure",
    "label": "Notify on Service Failure",
    "kind": "boolean",
    "hint": "Notify on Service Failure controls how this workflow behaves in production and during troubleshooting. For example, emit alerts when backend enters inactive state unexpectedly."
  }
];
  const actionFields: Field[] = [
  {
    ...
    "kind": "text",
    "hint": "Restart Reason controls how this workflow behaves in production and during troubleshooting. For example, include a change ticket number to link restarts with approved work."
  },
  {
    "key": "drainSeconds",
    "label": "Grace Drain Seconds",
    "kind": "number",
    "hint": "Grace Drain Seconds controls how this workflow behaves in production and during troubleshooting. For example, set 5-10 seconds to reduce disruption before service recycle."
  },
  {
    "key": "verifyPools",
    "label": "Verify Pool Utilization",
    "kind": "boolean",
    "hint": "Verify Pool Utilization controls how this workflow behaves in production and during troubleshooting. For example, run post-restart checks to confirm pools are readable and healthy."
  },
  {
    "key": "verifyLeases",
    "label": "Verify Lease Count",
    "kind": "boolean",
    "hint": "Verify Lease Count controls how this workflow behaves in production and during troubleshooting. For example, validate lease totals to ensure backend recovered with persisted state."
  },
  {
    "key": "backend",
    "label": "Backend Hint",
    ...
    "hint": "Backend Hint controls how this workflow behaves in production and during troubleshooting. For example, target backend-specific checks after restarts in mixed environments."
  },
  {
    "key": "notify",
    "label": "Send Notification",
    "kind": "boolean",
    "hint": "Send Notification controls how this workflow behaves in production and during troubleshooting. For example, announce controlled restarts to operations channels automatically."
  }
];
  const rowFields: Field[] = [
  {
    ...
    "kind": "text",
    "required": true,
    "hint": "Pool Name controls how this workflow behaves in production and during troubleshooting. For example, name pools by interface and subnet for easier lease troubleshooting."
  },
  {
    "key": "interface",
    "label": "Interface",
    ...
    "hint": "Interface controls how this workflow behaves in production and during troubleshooting. For example, assign pools per interface to prevent accidental overlap."
  },
  {
    "key": "rangeStart",
    "label": "Range Start",
    "kind": "text",
    "hint": "Range Start controls how this workflow behaves in production and during troubleshooting. For example, set clear boundaries to avoid conflicts with static infrastructure IPs."
  },
  {
    "key": "rangeEnd",
    "label": "Range End",
    "kind": "text",
    "hint": "Range End controls how this workflow behaves in production and during troubleshooting. For example, match subnet size and expected client density for stable operations."
  },
  {
    "key": "leaseTime",
    "label": "Lease Time Seconds",
    "kind": "number",
    "hint": "Lease Time Seconds controls how this workflow behaves in production and during troubleshooting. For example, shorter leases suit guest networks while longer leases fit stable office LANs."
  },
  {
    "key": "enabled",
    "label": "Enabled",
    "kind": "boolean",
    "hint": "Enabled controls how this workflow behaves in production and during troubleshooting. For example, disable underutilized pools during phased migration without deleting them."
  },
  {
    "key": "description",
    "label": "Description",
    "kind": "textarea",
    "hint": "Description controls how this workflow behaves in production and during troubleshooting. For example, document intended clients and VLAN purpose for audit clarity."
  }
];
  const secondaryTables: { endpoint: string; title: string; description: string; columns: string[] }[] = [
  {
    "endpoint": "/dhcp/service-status",
    "title": "DHCP Runtime Health",
    "description": "Service activity, backend, uptime, and memory indicators from the live DHCP daemon.",
    "columns": [
      "status",
      "backend",
      "uptime",
  ...
  },
  {
    ...
    "description": "Per-pool utilization with lease counts to identify exhaustion risks before impact.",
    "columns": [
      "pool",
      "interface",
      "total",
  ...
  async function loadSettings() {
    ...
    try {
      ...
      toasts.error($_('dhcp_status.toastfailed_to_load_configuration_settings'));
    }
  }
  ...
  async function loadRows() {
    ...
    try {
      ...
      rows = [];
      toasts.error($_('dhcp_status.toastfailed_to_load_records'));
    } finally {
      loadingRows = false;
    }
  }
  ...
  async function saveSettings() {
    ...
    try {
      ...
      toasts.success($_('dhcp_status.toastsettings_saved'));
    } catch {
      toasts.error($_('dhcp_status.toastfailed_to_save_settings'));
    } finally {
      saving = false;
    }
  }
  ...
  async function runPrimaryAction() {
    ...
    try {
      ...
      toasts.success($_('dhcp_status.toastaction_completed_successfully'));
      await loadSecondary();
    } catch {
      actionResults = [];
      actionOutput = 'Request failed. Check backend service status and try again.';
      toasts.error($_('dhcp_status.toastaction_failed'));
    } finally {
      runningAction = false;
    }
  }
  ...
  async function saveRow() {
    ...
    try {
      ...
      toasts.error($_('dhcp_status.toastfailed_to_save_record'));
    } finally {
      saving = false;
    }
  }
  ...
  async function deleteRow(id: string) {
    ...
    try {
      ...
      toasts.success($_('dhcp_status.toastrecord_deleted'));
      await Promise.all([loadRows(), loadSecondary()]);
    } catch {
      toasts.error($_('dhcp_status.toastfailed_to_delete_record'));
    } finally {
      saving = false;
    }
  }
  ...
  const summaryCards = $derived.by(() => [
    { label: 'Total Services', value: String(rows.length) },
    { label: 'Showing', value: String(filteredRows.length) },
    { label: 'Action Results', value: String(actionResults.length) },
    { label: 'Server Backends', value: String(secondaryTables.length + 2) }
  ]);
...
<div class="space-y-6">
  <Card class="border-slate-800 bg-slate-900/60">
    <CardHeader>
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <CardTitle class="text-slate-100">{$_('dhcp_status.dhcp_service_status')}</CardTitle>
          <CardDescription class="text-slate-400">{$_('dhcp_status.track_service_activity_backend_mode_pool_utilizati')}</CardDescription>
        </div>
    ...
    <CardContent class="space-y-6">
      ...
      {#if loading}
        <div class="rounded-md border border-slate-800 bg-slate-950/50 px-4 py-8 text-center text-sm text-slate-400">Loading DHCP status...</div>
      {:else}
        ...
        <Collapsible.Root bind:open={showAdvanced} class="pt-1">
          <Collapsible.Trigger>
            <span class="font-medium text-slate-200">{$_('dhcp_status.advanced_operational_behavior')}</span>
  ...
  <Card class="border-slate-800 bg-slate-900/60">
    <CardHeader>
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <CardTitle class="text-slate-100">{$_('dhcp_status.dhcp_service_control')}</CardTitle>
          <CardDescription class="text-slate-400">{$_('dhcp_status.perform_safe_restart_actions_and_immediate_postche')}</CardDescription>
        </div>
        <Button class="bg-cyan-600 text-white hover:bg-cyan-700" onclick={runPrimaryAction} disabled={runningAction || loading || !actionEndpoint}>
          <Play class="mr-2 h-4 w-4" /> {runningAction ? 'Running...' : 'Restart DHCP Service'}
        </Button>
      </div>
    </CardHeader>
    <CardContent class="space-y-4">
      ...
      <Collapsible.Root bind:open={showToolHints}>
        <Collapsible.Trigger>
          <span class="font-medium text-slate-200">{$_('dhcp_status.how_to_interpret_tool_output')}</span>
      ...
      <div class="rounded-md border border-slate-800 bg-slate-950/50 p-3">
        <p class="mb-2 text-xs uppercase tracking-wide text-slate-400">{$_('dhcp_status.live_output')}</p>
  ...
  <Card class="border-slate-800 bg-slate-900/60">
    <CardHeader>
      <CardTitle class="text-slate-100">{$_('dhcp_status.dhcp_service_status_1')}</CardTitle>
      <CardDescription class="text-slate-400">{$_('dhcp_status.create_edit_and_delete_records_directly_in_place_t')}</CardDescription>
    </CardHeader>
    <CardContent class="space-y-4">
      <div class="grid gap-3 md:grid-cols-3">
        <Input class="border-slate-700 bg-slate-950 text-slate-100" placeholder={$_('dhcp_status.placeholdersearch_records')} value={search} oninput={(event) => (search = (event.currentTarget as HTMLInputElement).value)} />
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              {#if (secondaryData[section.endpoint] ?? []).length === 0}
                <tr><td class="px-3 py-6 text-center text-slate-500" colspan={section.columns.length}>No DHCP status found.</td></tr>
              {:else}
