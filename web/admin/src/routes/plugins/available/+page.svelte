<!-- Route view for `/plugins/available` in the ezNGFW admin GUI. -->
...
<script lang="ts">
  ...
  const rowsEndpoint = "/plugins/available";
  const actionEndpoint = "/plugins/refresh";

  const settingsFields: Field[] = [
  {
    "key": "autoRefresh",
    "label": "Auto Refresh Catalog",
    "kind": "boolean",
    "hint": "Auto Refresh Catalog controls how this workflow behaves in production and during troubleshooting. For example, enable hourly sync in lab environments and disable in tightly controlled production change windows."
  },
  {
    "key": "refreshSeconds",
    "label": "Refresh Interval Seconds",
    "kind": "number",
    "hint": "Refresh Interval Seconds controls how this workflow behaves in production and during troubleshooting. For example, set 3600 for hourly sync or 86400 for daily package checks."
  },
  {
    "key": "defaultCategory",
    ...
    "hint": "Default Category Filter controls how this workflow behaves in production and during troubleshooting. For example, start in security category during hardening initiatives and all categories for general maintenance."
  },
  {
    "key": "showDependencies",
    "label": "Show Dependencies",
    "kind": "boolean",
    "hint": "Show Dependencies controls how this workflow behaves in production and during troubleshooting. For example, keep this enabled to avoid installs that conflict with pinned package versions."
  },
  {
    "key": "showBeta",
    "label": "Include Beta Packages",
    "kind": "boolean",
    "hint": "Include Beta Packages controls how this workflow behaves in production and during troubleshooting. For example, turn on only in staging environments where regression testing is expected."
  },
  {
    "key": "autoSnapshot",
    "label": "Snapshot Before Install",
    "kind": "boolean",
    "hint": "Snapshot Before Install controls how this workflow behaves in production and during troubleshooting. For example, create rollback points before plugin install to reduce downtime risk."
  }
];
  const actionFields: Field[] = [
  {
    ...
    "label": "Search Query",
    "kind": "text",
    "hint": "Search Query controls how this workflow behaves in production and during troubleshooting. For example, search dns to narrow the list to resolver and dnssec add-ons quickly."
  },
  {
    "key": "category",
    "label": "Category",
    ...
    "hint": "Category controls how this workflow behaves in production and during troubleshooting. For example, select monitoring when evaluating telemetry-focused plugins for NOC teams."
  },
  {
    "key": "includeDependencies",
    "label": "Include Dependency Graph",
    "kind": "boolean",
    "hint": "Include Dependency Graph controls how this workflow behaves in production and during troubleshooting. For example, enable this before production rollout to verify transitive package impact."
  },
  {
    "key": "includeVersions",
    "label": "Show Version Matrix",
    "kind": "boolean",
    "hint": "Include Version Matrix controls how this workflow behaves in production and during troubleshooting. For example, compare installed and candidate versions before approving changes."
  },
  {
    "key": "onlyUpgradeable",
    "label": "Only Upgradeable",
    "kind": "boolean",
    "hint": "Only Upgradeable controls how this workflow behaves in production and during troubleshooting. For example, filter to upgrade candidates during patch cycles to reduce review noise."
  },
  {
    "key": "limit",
    "label": "Result Limit",
    "kind": "number",
    "hint": "Result Limit controls how this workflow behaves in production and during troubleshooting. For example, use 200 for full inventory exports and 30 for quick operator lookups."
  }
];
  const rowFields: Field[] = [
  {
    ...
    "kind": "text",
    "required": true,
    "hint": "Plugin Name controls how this workflow behaves in production and during troubleshooting. For example, store package identifiers exactly so API install calls resolve correctly."
  },
  {
    "key": "version",
    "label": "Version",
    "kind": "text",
    "hint": "Version controls how this workflow behaves in production and during troubleshooting. For example, track pin-worthy versions when vendor advisories require fixed baselines."
  },
  {
    "key": "category",
    "label": "Category",
    ...
    "hint": "Category controls how this workflow behaves in production and during troubleshooting. For example, set accurate categories so operators can filter large catalogs efficiently."
  },
  {
    "key": "dependencies",
    "label": "Dependencies",
    "kind": "text",
    "hint": "Dependencies controls how this workflow behaves in production and during troubleshooting. For example, list core dependencies to surface change-control risk before installation."
  },
  {
    "key": "enabled",
    "label": "Install Requested",
    "kind": "boolean",
    "hint": "Install Requested controls how this workflow behaves in production and during troubleshooting. For example, toggle install intent before scheduled maintenance execution."
  },
  {
    "key": "description",
    "label": "Description",
    "kind": "textarea",
    "hint": "Description controls how this workflow behaves in production and during troubleshooting. For example, add deployment notes like required reboot or service restart expectations."
  }
];
  const secondaryTables: { endpoint: string; title: string; description: string; columns: string[] }[] = [
  {
    "endpoint": "/plugins/available",
    "title": "Catalog View",
    "description": "Current plugin inventory including description, version, category, and dependency metadata.",
    "columns": [
      "name",
      "version",
      "category",
  ...
  },
  {
    ...
    "description": "Installed package state to compare against available versions and plan upgrades safely.",
    "columns": [
      "name",
      "version",
      "status",
  ...
  async function loadSettings() {
    ...
    try {
      ...
      toasts.error($_('plugins_available.toastfailed_to_load_configuration_settings'));
    }
  }
  ...
  async function loadRows() {
    ...
    try {
      ...
      rows = [];
      toasts.error($_('plugins_available.toastfailed_to_load_records'));
    } finally {
      loadingRows = false;
    }
  }
  ...
  async function saveSettings() {
    ...
    try {
      ...
      toasts.success($_('plugins_available.toastsettings_saved'));
    } catch {
      toasts.error($_('plugins_available.toastfailed_to_save_settings'));
    } finally {
      saving = false;
    }
  }
  ...
  async function runPrimaryAction() {
    ...
    try {
      ...
      toasts.success($_('plugins_available.toastaction_completed_successfully'));
      await loadSecondary();
    } catch {
      actionResults = [];
      actionOutput = 'Request failed. Check backend service status and try again.';
      toasts.error($_('plugins_available.toastaction_failed'));
    } finally {
      runningAction = false;
    }
  }
  ...
  async function saveRow() {
    ...
    try {
      ...
      toasts.error($_('plugins_available.toastfailed_to_save_record'));
    } finally {
      saving = false;
    }
  }
  ...
  async function deleteRow(id: string) {
    ...
    try {
      ...
      toasts.success($_('plugins_available.toastrecord_deleted'));
      await Promise.all([loadRows(), loadSecondary()]);
    } catch {
      toasts.error($_('plugins_available.toastfailed_to_delete_record'));
    } finally {
      saving = false;
    }
  }
  ...
  async function installPlugin(name: string) {
    ...
    try {
      ...
      toasts.success(`Install requested for ${name}`);
      await loadSecondary();
    } catch {
      toasts.error($_('plugins_available.toastfailed_to_request_plugin_install'));
    } finally {
      saving = false;
    }
  }
  ...
  const summaryCards = $derived.by(() => [
    { label: 'Total Plugins', value: String(rows.length) },
    { label: 'Showing', value: String(filteredRows.length) },
    { label: 'Installed', value: String(rows.filter(r => r.installed || r.status === 'installed').length) },
    { label: 'Available', value: String(rows.filter(r => !r.installed && r.status !== 'installed').length) }
  ]);
...
<div class="space-y-6">
  <Card class="border-slate-800 bg-slate-900/60">
    <CardHeader>
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <CardTitle class="text-slate-100">{$_('plugins_available.available_plugin_catalog')}</CardTitle>
          <CardDescription class="text-slate-400">{$_('plugins_available.search_installable_packages_with_category_and_depe')}</CardDescription>
        </div>
    ...
    <CardContent class="space-y-6">
      ...
      {#if loading}
        <div class="rounded-md border border-slate-800 bg-slate-950/50 px-4 py-8 text-center text-sm text-slate-400">Loading plugins...</div>
      {:else}
        ...
        <Collapsible.Root bind:open={showAdvanced} class="pt-1">
          <Collapsible.Trigger>
            <span class="font-medium text-slate-200">{$_('plugins_available.advanced_operational_behavior')}</span>
  ...
  <Card class="border-slate-800 bg-slate-900/60">
    <CardHeader>
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <CardTitle class="text-slate-100">{$_('plugins_available.catalog_sync')}</CardTitle>
          <CardDescription class="text-slate-400">{$_('plugins_available.synchronize_package_metadata_with_upstream_reposit')}</CardDescription>
        </div>
        <Button class="bg-cyan-600 text-white hover:bg-cyan-700" onclick={runPrimaryAction} disabled={runningAction || loading || !actionEndpoint}>
          <Play class="mr-2 h-4 w-4" /> {runningAction ? 'Running...' : 'Refresh Catalog'}
        </Button>
      </div>
    </CardHeader>
    <CardContent class="space-y-4">
      ...
      <Collapsible.Root bind:open={showToolHints}>
        <Collapsible.Trigger>
          <span class="font-medium text-slate-200">{$_('plugins_available.how_to_interpret_tool_output')}</span>
      ...
      <div class="rounded-md border border-slate-800 bg-slate-950/50 p-3">
        <p class="mb-2 text-xs uppercase tracking-wide text-slate-400">{$_('plugins_available.live_output')}</p>
  ...
  <Card class="border-slate-800 bg-slate-900/60">
    <CardHeader>
      <CardTitle class="text-slate-100">{$_('plugins_available.available_plugins')}</CardTitle>
      <CardDescription class="text-slate-400">{$_('plugins_available.create_edit_and_delete_records_directly_in_place_t')}</CardDescription>
    </CardHeader>
    <CardContent class="space-y-4">
      <div class="grid gap-3 md:grid-cols-3">
        <Input class="border-slate-700 bg-slate-950 text-slate-100" placeholder={$_('plugins_available.placeholdersearch_records')} value={search} oninput={(event) => (search = (event.currentTarget as HTMLInputElement).value)} />
            ...
            {:else}
              {#each filteredRows as row}
                <tr class="border-t border-slate-800/80 text-slate-200">
                  ...
                  <td class="px-3 py-2">
                    <div class="flex gap-2">
                      <Button size="sm" variant="outline" class="border-cyan-500/50 text-cyan-300 hover:bg-cyan-950/40" onclick={() => void installPlugin(String(row.name ?? row.id ?? ''))}>
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              {#if (secondaryData[section.endpoint] ?? []).length === 0}
                <tr><td class="px-3 py-6 text-center text-slate-500" colspan={section.columns.length}>No plugins found.</td></tr>
              {:else}
