<!-- Route view for `/syslog/viewer` in the ezNGFW admin GUI. -->
...
<script lang="ts">
  ...
  const secondaryTables = [
    {
        "endpoint": "/syslog/viewer",
        "title": "Live Logs",
        "description": "Current log lines returned by viewer backend.",
        "columns": [
            "timestamp",
            "source",
            "level",
    ...
    {
        "endpoint": "/syslog/viewer/stats",
        "title": "Ingestion Statistics",
        "description": "Pipeline ingestion and buffering stats.",
        "columns": [
            "window",
            "lines",
            "drops",
  ...
  async function loadSettings() {
    try {
      const payload = await api.get<Record<string, any>>('/syslog/viewer/settings');
      ...
      toasts.error($_('syslog_viewer.toastfailed_to_load_page_settings'));
    }
  }
  ...
  async function loadRows() {
    loadingRows = true;
    try {
      const payload = await api.get<unknown[]>('/syslog/viewer/filters');
      rows = Array.isArray(payload) ? payload.map((entry) => normalizeRecord(entry, rowFields)) : [];
    } catch {
      rows = [];
      toasts.error($_('syslog_viewer.toastfailed_to_load_inline_records'));
    } finally {
      loadingRows = false;
    }
  }
  ...
  async function saveSettings() {
    ...
    try {
      ...
      await api.put('/syslog/viewer/settings', payload);
      toasts.success($_('syslog_viewer.toastsettings_saved'));
    } catch {
      toasts.error($_('syslog_viewer.toastfailed_to_save_settings'));
    } finally {
      saving = false;
    }
  }
  ...
  async function saveRow() {
    if (!validateDraft()) {
      toasts.error($_('syslog_viewer.toastplease_resolve_validation_errors_before_savin'));
      return;
    }
    ...
    try {
      ...
      if (editingId) {
        await api.put(`/syslog/viewer/filters/${editingId}`, payload);
      } else {
        await api.post('/syslog/viewer/filters', payload);
      }
      toasts.success(editingId ? 'Profile updated' : 'Profile added');
      resetDraft();
      await Promise.all([loadRows(), loadSecondary()]);
    } catch {
      toasts.error($_('syslog_viewer.toastfailed_to_save_profile'));
    } finally {
      saving = false;
    }
  }
  ...
  async function deleteRow(id: string) {
    deleting = true;
    try {
      await api.del(`/syslog/viewer/filters/${id}`);
      toasts.success($_('syslog_viewer.toastprofile_deleted'));
      await Promise.all([loadRows(), loadSecondary()]);
    } catch {
      toasts.error($_('syslog_viewer.toastfailed_to_delete_profile'));
    } finally {
      deleting = false;
    }
  }
...
<div class="space-y-6">
  <div class="grid gap-4 md:grid-cols-3">
    <Card class="border-slate-800 bg-slate-900/60">
      <CardHeader class="pb-2"><CardTitle class="text-sm text-slate-200">{$_('syslog_viewer.active_profiles')}</CardTitle></CardHeader>
      <CardContent><p class="text-2xl font-semibold text-cyan-400">{activeCount}</p></CardContent>
    </Card>
    <Card class="border-slate-800 bg-slate-900/60">
      <CardHeader class="pb-2"><CardTitle class="text-sm text-slate-200">{$_('syslog_viewer.disabled_profiles')}</CardTitle></CardHeader>
      <CardContent><p class="text-2xl font-semibold text-amber-300">{disabledCount}</p></CardContent>
    </Card>
    <Card class="border-slate-800 bg-slate-900/60">
      <CardHeader class="pb-2"><CardTitle class="text-sm text-slate-200">{$_('syslog_viewer.validation_issues')}</CardTitle></CardHeader>
  ...
  <Card class="border-slate-800 bg-slate-900/60">
    <CardHeader>
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <CardTitle class="text-slate-100">{$_('syslog_viewer.syslog_viewer_and_filter_profiles')}</CardTitle>
          <CardDescription class="text-slate-400">{$_('syslog_viewer.build_richer_filter_profiles_tune_ingestion_contro')}</CardDescription>
        </div>
    ...
    <CardContent class="space-y-6">
      {#if loading}
        <div class="rounded-md border border-slate-800 bg-slate-950/50 px-4 py-8 text-center text-sm text-slate-400">Loading syslog entries...</div>
      {:else}
        ...
        <Collapsible.Root bind:open={showAdvanced} class="pt-1">
          <Collapsible.Trigger class="flex w-full items-center justify-between rounded-md border border-slate-800 bg-slate-950/40 px-3 py-2">
            <span class="font-medium text-slate-200">{$_('syslog_viewer.advanced_operational_behavior')}</span>
  ...
  <Card class="border-slate-800 bg-slate-900/60">
    <CardHeader>
      <CardTitle class="text-slate-100">{$_('syslog_viewer.log_filter_profiles')}</CardTitle>
      <CardDescription class="text-slate-400">{$_('syslog_viewer.create_validate_edit_and_delete_records_directly_w')}</CardDescription>
    </CardHeader>
    <CardContent class="space-y-4">
      <div class="grid gap-3 md:grid-cols-5">
        <Input class="border-slate-700 bg-slate-950 text-slate-100 md:col-span-2" placeholder={$_('syslog_viewer.placeholdersearch_profiles_by_any_field')} value={search} oninput={(event) => (search = (event.currentTarget as HTMLInputElement).value)} />
      ...
      {#if Object.keys(validation).length > 0}
        <div class="rounded-md border border-rose-500/40 bg-rose-950/20 px-3 py-2 text-sm text-rose-200">
          Fix highlighted fields before saving this log filter profile. Validation runs inline to prevent malformed profile payloads.
        </div>
      {/if}
      ...
      <div class="rounded-md border border-slate-800 bg-slate-950/40 p-3">
        <button type="button" class="flex w-full items-center justify-between text-sm text-slate-200" onclick={() => (showFormAdvanced = !showFormAdvanced)}>
          <span>{$_('syslog_viewer.advanced_settings_for_log_filter_profile_profile')}</span>
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              {#if (secondaryData[section.endpoint] ?? []).length === 0}
                <tr><td class="px-3 py-6 text-center text-slate-500" colspan={section.columns.length}>No syslog entries found.</td></tr>
              {:else}
